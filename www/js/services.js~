...
.factory('Chats', function($http,chatappdb,$rootScope) {
  ...
  return {
    ...
    history: function() {
             //return the promise directly.
             return $http.get(host+'/offlinemessages')
...
self.update_user_byemail = function(email,status) {
  //console.log("update  friend_lists set status='"+status+"' where email='"+email+"' ");
    return DBA.query("update  friend_lists set status=? where email=? ",[status,email])
...
self.gethistory = function(id,name) {
 ...
 self.remove_user = function() {
  ...
  // DBA.query("CREATE TABLE IF NOT EXISTS friend_lists (_id text, display_name text,email text,pic text ,status text)");
	...
	for(var i = 0; i < json.length; i++) {
		...
		DBA.query("INSERT INTO friend_lists ('_id', 'display_name','email','pic','status') VALUES (?,?,?,?,?)",[user_id,user_name,user_email,user_face,status]).then(function (res) {
